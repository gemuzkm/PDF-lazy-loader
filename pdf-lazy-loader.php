...
// Priority 999 — dequeue whatever was registered before shortcodes ran
add_action( 'wp_enqueue_scripts', 'pdf_lazy_loader_dequeue_pdfemb_assets', 999 );

// Priority 1 — after ALL shortcodes executed, scan registered assets again
add_action( 'wp_footer', 'pdf_lazy_loader_scan_all_pdfemb_registered', 1 );
// Priority 2 — inject the final merged list as window.pdfLazyLoaderLateAssets
add_action( 'wp_footer', 'pdf_lazy_loader_inject_assets_to_footer', 2 );

// ob_start: strip any surviving PDF Embedder tags from the HTML stream
...
// Matches: /PDFEmbedder-premium/, /PDFEmbedder-premium-secure/, /pdf-embedder/, /pdfemb/
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Layer 1 — dequeue at wp_enqueue_scripts:999
// Catches assets registered BEFORE shortcodes run (e.g. via plugins_loaded)
// ---------------------------------------------------------------------------
function pdf_lazy_loader_get_pdfemb_handles() {
    return array(
        'styles' => array(
            'pdfemb-fullscreen', 'pdfemb-frontend', 'pdfemb-pdf-fullscreen',
            'pdf-fullscreen', 'pdf-embedder', 'pdfemb',
            'pdfemb-all-premium', 'pdfemb-print',
        ),
        'scripts' => array(
...
function pdf_lazy_loader_dequeue_pdfemb_assets() {
    ...
    // Dequeue anything in the queue matching pdfemb path
    foreach ( (array) $wp_styles->queue as $h ) {
        if ( ! isset( $wp_styles->registered[ $h ] ) ) continue;
        $src = $wp_styles->registered[ $h ]->src;
        if ( pdf_lazy_loader_is_pdfemb_src( $src ) ) {
            if ( strpos( $src, '//' ) !== 0 && strpos( $src, 'http' ) !== 0 ) $src = site_url( $src );
            if ( ! in_array( $src, $css_urls, true ) ) $css_urls[] = $src;
            wp_dequeue_style( $h );
    ...
    foreach ( (array) $wp_scripts->queue as $h ) {
        ...
        if ( pdf_lazy_loader_is_pdfemb_src( $src ) ) {
            if ( strpos( $src, '//' ) !== 0 && strpos( $src, 'http' ) !== 0 ) $src = site_url( $src );
            if ( ! in_array( $src, $js_urls, true ) ) $js_urls[] = $src;
            wp_dequeue_script( $h );
...
// ---------------------------------------------------------------------------
// Layer 2 — scan ALL registered assets at wp_footer:1
// At this point ALL shortcodes have executed, so pdfemb-fullscreen etc. are
// fully registered. We collect every style/script whose src lives in a
// PDF Embedder plugin directory and dequeue/deregister it.
// ---------------------------------------------------------------------------
function pdf_lazy_loader_scan_all_pdfemb_registered() {
    ...
    $css_urls = isset( $GLOBALS['pdf_lazy_loader_pdfemb_assets']['css'] )
        ? $GLOBALS['pdf_lazy_loader_pdfemb_assets']['css']
        : array();
    $js_urls  = isset( $GLOBALS['pdf_lazy_loader_pdfemb_assets']['js'] )
        ? $GLOBALS['pdf_lazy_loader_pdfemb_assets']['js']
        : array();

    // Scan ALL registered styles
    foreach ( $wp_styles->registered as $handle => $dep ) {
        $src = $dep->src;
        if ( ! $src || ! pdf_lazy_loader_is_pdfemb_src( $src ) ) continue;
        if ( strpos( $src, '//' ) !== 0 && strpos( $src, 'http' ) !== 0 ) $src = site_url( $src );
        if ( ! in_array( $src, $css_urls, true ) ) $css_urls[] = $src;
        wp_dequeue_style( $handle );
        wp_deregister_style( $handle );
...
// ---------------------------------------------------------------------------
// Layer 3 — inject final asset list into footer as window.pdfLazyLoaderLateAssets
// Runs at wp_footer:2, right after the scan above.
// This is the most reliable channel: JS reads this global on click and
// merges it with pdfLazyLoaderData.pdfembAssets (early dequeue list).
// ---------------------------------------------------------------------------
function pdf_lazy_loader_inject_assets_to_footer() {
    if ( is_admin() ) return;
    if ( ! pdf_lazy_loader_has_pdf_iframes() ) return;

    $assets = isset( $GLOBALS['pdf_lazy_loader_pdfemb_assets'] )
        ? $GLOBALS['pdf_lazy_loader_pdfemb_assets']
        : array( 'css' => array(), 'js' => array() );

    if ( empty( $assets['css'] ) && empty( $assets['js'] ) ) return;

    echo '<script type="text/javascript">window.pdfLazyLoaderLateAssets=' . wp_json_encode( $assets ) . ';</script>' . "\n";
}

// ---------------------------------------------------------------------------
// ob_start — strip surviving PDF Embedder <link>/<script> from HTML stream
// Broadened pattern: matches any href/src whose path contains a PDF Embedder
// directory segment (case-insensitive). Captures URLs before stripping.
// ---------------------------------------------------------------------------
...
function pdf_lazy_loader_filter_html_output( $html ) {
    ...
    // Broad path-based pattern — catches any file from a PDF Embedder plugin dir
    $css_pattern = '#<link\b[^>]*\bhref=["\']([^"\']*(?:/PDFEmbedder-premium(?:-secure)?|/pdf-embedder(?:-premium)?|/pdfemb)[^"\']*)["\'][^>]*/?>\s*#i';
    $js_pattern  = '#<script\b[^>]*\bsrc=["\']([^"\']*(?:/PDFEmbedder-premium(?:-secure)?|/pdf-embedder(?:-premium)?|/pdfemb)[^"\']*)["\'][^>]*>\s*</script>#i';

    $ob_css = array();
    $ob_js  = array();

    if ( preg_match_all( $css_pattern, $html, $m ) ) {
        $ob_css = array_values( array_unique( $m[1] ) );
    }
    if ( preg_match_all( $js_pattern, $html, $m ) ) {
        $ob_js = array_values( array_unique( $m[1] ) );
    }

    // Merge into global (footer injection already ran but ob_start fires last)
    if ( ! empty( $ob_css ) || ! empty( $ob_js ) ) {
        $existing = isset( $GLOBALS['pdf_lazy_loader_pdfemb_assets'] )
            ? $GLOBALS['pdf_lazy_loader_pdfemb_assets']
            : array( 'css' => array(), 'js' => array() );
    ...
    // Strip the tags
    $html = preg_replace( $css_pattern, '', $html );
    $html = preg_replace( $js_pattern,  '', $html );
...
function pdf_lazy_loader_add_admin_menu() {
    add_options_page(
        'PDF Lazy Loader Settings', 'PDF Lazy Loader', 'manage_options',
...
function pdf_lazy_loader_register_settings() {
    ...
    register_setting( 'pdf_lazy_loader_settings', 'pdf_lazy_loader_loading_time',            array( 'type' => 'integer', 'sanitize_callback' => 'absint',             'default' => 1500 ) );
                ...
                for(var m=0;m<muts.length;m++){
                    ...
                    for(var n=0;n<nodes.length;n++){
                        ...
                        if(node.querySelectorAll){
                            ...
                            for(var k=0;k<iframes.length;k++) intercept(iframes[k]);
                        }
                    }
                }
            });
...
function pdf_lazy_loader_enqueue_frontend_scripts() {
    ...
    add_action( 'wp_enqueue_scripts', function() use ( $settings ) {
        // pdfembAssets at this point may be partial (shortcodes not yet run).
        // The complete list will be injected into window.pdfLazyLoaderLateAssets
        // by pdf_lazy_loader_inject_assets_to_footer() at wp_footer:2.
            ...
            <div class="pll-section">
                ...
                <table class="form-table pll-table">
                    ...
                    <tr>
                        ...
                        <td>
                            <input type="number" id="pdf_lazy_loader_loading_time" name="pdf_lazy_loader_loading_time" value="<?php echo esc_attr( $settings['loadingTime'] ); ?>" min="500" max="5000" step="100" class="pll-input-sm" />
